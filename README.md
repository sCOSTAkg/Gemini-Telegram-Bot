...
### 启动与集成管理（Railway 网页面板）
1. 构建完成后，在服务的 **Deployments** 面板点击 **Redeploy** 或 **Start**，即可启动机器人进程。
2. 在 **Variables** 标签页可以随时更新 `TELEGRAM_BOT_API_KEY` 和 `GOOGLE_GEMINI_KEY`，保存后点击 **Deployments → Deploy now** 以应用新的集成配置。
3. 如需为机器人添加命令菜单，前往 [BotFather](https://t.me/BotFather) 使用 `/setcommands`，把常用命令（如 `gemini`, `draw`, `edit`, `switch`, `clear`）填入，Railway 会在下次启动时自动加载。
4. 使用 **Metrics / Logs** 页面可在线查看运行日志，必要时通过 **Actions → Restart service** 在网页面板中重启或停止机器人。
5. 若想要扩展额外服务（例如数据库或缓存），可以在 Railway 的同一项目中添加新服务并通过环境变量暴露连接信息给机器人，无需改动代码即可完成集成。
